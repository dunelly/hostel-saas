...
export async function POST(request: NextRequest) {
  try {
    ...
    const { reservationId, newBedId, fromBedId, singleDate } = parsed.data;
    ...
    // Get the assignments to move — scope to fromBedId to avoid moving all beds of a multi-guest reservation
    const conditions = [eq(bedAssignments.reservationId, reservationId)];
    if (fromBedId) {
      conditions.push(eq(bedAssignments.bedId, fromBedId));
    }
    if (singleDate) {
