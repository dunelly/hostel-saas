...
**Goal:** Restyle GuestDetailPanel from card-based layout to Opera PMS-style dense form-grid with dark title bar, 2-column grid, payment strip, and compact toolbar.

**Architecture:** Pure JSX/styling rewrite of a single component. No logic, state, mutation, or prop changes. Lines 1-301 (imports, interfaces, hooks, handlers) stay untouched. Lines 302-1088 (return JSX) get completely replaced. The sub-flows (check-in form, checkout confirm, guest edit, extend stay, bill view) keep their behavior but adopt the new dense styling.

**Tech Stack:** React, Tailwind CSS, Lucide icons (already imported)

---

### Task 1: Replace the entire return JSX with Opera PMS layout

**Files:**
- Modify: `apps/web/src/components/GuestDetailPanel.tsx:302-1088`

This task replaces everything from `return (` to the end of the component with the new Opera PMS-style layout.

- [ ] **Step 1: Read the current file to confirm line numbers**

Run: `head -305 apps/web/src/components/GuestDetailPanel.tsx | tail -5`
Expected: Shows lines around `return (` at line 302.

- [ ] **Step 2: Replace the entire return statement**

Replace everything from line 302 (`return (`) through line 1088 (`}`) with the new Opera PMS layout below.

The new return JSX should be:
...
```bash
git add apps/web/src/components/GuestDetailPanel.tsx
git commit -m "feat: redesign guest panel to Opera PMS style (dense form-grid + toolbar)"
```
...
**Files:**
- Modify: `apps/web/src/components/GuestDetailPanel.tsx:1-10,280`

After the JSX rewrite, some imports and variables are no longer used:

- [ ] **Step 1: Remove unused imports**

The following icons are no longer used in the new layout: `CalendarPlus`, `Receipt`, `ChevronDown`, `CreditCard`. The `Moon` icon IS still used in the extend form. `LogIn`, `LogOut`, `Ban`, `UserX` are no longer used (status icons removed from title bar).
...
- [ ] **Step 2: Remove unused `initials` variable**
...
- [ ] **Step 3: Remove unused `StatusIcon` variable**
...
- [ ] **Step 4: Simplify STATUS_CFG (remove unused fields)**
...
```bash
git add apps/web/src/components/GuestDetailPanel.tsx
git commit -m "chore: remove unused imports and variables from guest panel"
```
